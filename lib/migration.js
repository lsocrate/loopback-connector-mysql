...
function mixinMigration(MySQL, mysql) {
  ...
  MySQL.prototype.autoupdate = function(models, cb) {
    ...
    async.each(models, function(model, done) {
      ...
      self.execute('SHOW FULL FIELDS FROM ' + table, function(err, fields) {
  ...
  MySQL.prototype.alterTable = function(model, actualFields, actualIndexes, done, checkOnly) {
    ...
    function actualize(propName, oldSettings) {
      ...
      if (newSettings && changed(newSettings, oldSettings, propName)) {
        var pName = self.client.escapeId(propName);
    ...
    function changed(newSettings, oldSettings, propName) {
      ...
      var columnMetadata = self.columnMetadata(model, propName);
      // The simple comparison accounts for matching null and undefined as true
      if (!columnMetadata || oldSettings.Collation != columnMetadata.collation) {
        return true;
      }
  ...
  MySQL.prototype.buildColumnDefinition = function(model, prop) {
    ...
    var collation = p.mysql && p.mysql.collation;
